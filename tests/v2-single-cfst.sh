...
CFIP_LIB_DIR="$ROOT/package/luci-app-cloudflare-ip/root/usr/libexec/cf-ip" CFIP_WORK_DIR="$TMP" CFIP_RUNTIME_DIR="$TMP/runtime" CFIP_STATUS_DIR="$TMP/status" CFIP_LOG_FILE="$TMP/log" bash -c '
  ...
  CFIP_RUN_ID=test; CFIP_SPEEDTEST_PROTOCOL=tcp; CFIP_SPEEDTEST_THREADS=20; CFIP_SPEEDTEST_PING_COUNT=2; CFIP_SPEEDTEST_DN=2; CFIP_SPEEDTEST_DT=2; CFIP_SPEEDTEST_TLL=0; CFIP_SPEEDTEST_TL=""; CFIP_PROBE_TOP_COUNT=3; CFIP_MEASUREMENT_DEADLINE=$(( $(date +%s) + 20 ))
