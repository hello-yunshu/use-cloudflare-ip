...
jq -e '.partitions["cloudflare-ip"].handlerSnapshot.stateGeneration==3 and (.partitions["cloudflare-ip"].handlerSnapshot.state|implode|fromjson).feedback==1 and (.partitions["cloudflare-ip"].handlerSnapshot.state|implode|fromjson).actions["104.16.1.1"].samples==1 and ((.partitions["cloudflare-ip"].handlerSnapshot.state|implode|fromjson).weights[0] > 0.00095 and (.partitions["cloudflare-ip"].handlerSnapshot.state|implode|fromjson).weights[0] < 0.00098) and ((.partitions["cloudflare-ip"].handlerSnapshot.state|implode|fromjson).weights[1] > 0.0191 and (.partitions["cloudflare-ip"].handlerSnapshot.state|implode|fromjson).weights[1] < 0.0194)' "$CFIP_RILL_STATE" >/dev/null
