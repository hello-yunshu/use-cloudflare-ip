...
export CFIP_RUN_ID=shell-feedback-run CFIP_LOG_FILE="$TMP/log" CFIP_RILL_ENABLED=true CFIP_RILL_MODE=shadow CFIP_RILL_RUNTIME="$BIN" CFIP_RILL_STATE="$TMP/state.json" CFIP_RILL_TIMEOUT_S=5
...
jq --arg runId "$CFIP_RUN_ID" --argjson generation "$(jq '.generation' "$TMP/rill.json")" '{schemaVersion:1,runId:$runId,decisionId:.decisionId,selectedArm:.selectedArm,generation:$generation}' "$TMP/rill.json" >"$TMP/decision.json"
...
jq -e '.handlerSnapshot.stateGeneration==2 and (.handlerSnapshot.state|implode|fromjson).feedback==1 and (.handlerSnapshot.state|implode|fromjson).actionStats["0"].samples==1' "$CFIP_RILL_STATE" >/dev/null
