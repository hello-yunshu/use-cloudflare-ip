...
jq -e --arg commit "$commit" '.schemaVersion==1 and .commit==$commit and (.runId|length)>0 and (.jobs|type)=="object" and (.artifacts|type)=="array" and (.assetFiles|type)=="array" and (.rill|type)=="object" and (.releaseEligible|type)=="boolean" and ((.releaseEligible==false) or (.rill.schemaVersion==1 and .rill.package.repository=="hello-yunshu/rill-openwrt-packages" and (.rill.package.commit|test("^[0-9a-fA-F]{40}$")) and (.rill.package.qualificationRunId|type)=="number" and (.rill.package.qualificationManifestSha256|test("^[0-9a-fA-F]{64}$")) and (.rill.runtime.version|type)=="string" and (.rill.runtime.tag|type)=="string" and (.rill.runtime.commit|test("^[0-9a-fA-F]{40}$")) and (.rill.runtime.sourceArchiveSha256|test("^[0-9a-fA-F]{64}$")) and (.rill.runtime.binarySha256|test("^[0-9a-fA-F]{64}$")) and .rill.integration.status=="pass" and .rill.integration.sameRelease==true))' "$OUTPUT" >/dev/null
