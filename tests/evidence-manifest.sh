...
jq -e --arg commit "$commit" '.schemaVersion==1 and .commit==$commit and (.runId|length)>0 and (.jobs|type)=="object" and (.artifacts|type)=="array" and (.assetFiles|type)=="array" and (.rill|type)=="object" and (.releaseEligible|type)=="boolean"' "$OUTPUT" >/dev/null
