...
test "$(CFIP_SOURCE_POLICY=community-heavy CFIP_SOURCE_POLICY_EFFECTIVE=community-heavy cfip_source_policy_order "$sources")" = 'lancelotrar-best-cf-ipv4 cloudflare-official-v4 dustinwin-bestcf-cmcc svips-best-ips'
test "$(cfip_builtin_source_json dustinwin-bestcf-cmcc | jq -r .sourceClass)" = carrier
test "$(cfip_builtin_source_json svips-best-ips | jq -r .sourceClass)" = measured
