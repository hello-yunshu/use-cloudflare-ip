...
The measurement policy is explicit and bounded: `balanced`, `official-heavy`, `history-heavy`, `diversity-heavy`, or `community-heavy`. Active probes run in bounded batches with a deterministic early-stop rule and report considered, probed, and avoided candidates. Rill Reward v2 is bounded to `[-1,1]` and includes total latency, TTFB, worst-domain loss, throughput, and delayed stability. Delayed feedback persists across restart, re-probes the original candidate against its domain set, deduplicates entries, and quarantines corrupt queues. Rolling qualification retains at most 50 observations and requires attribution, delayed coverage, Native/Rill comparison, recent-error limits, and healthy Runtime Health/Inspect/model/schema gates; failure automatically falls back to Shadow/Native safety.
