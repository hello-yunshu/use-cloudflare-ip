...
`enable`, `disable`, `start`, `stop`, `restart`, `reload`, validation, cron cleanup, startup delay, CFST persistence and status updates remain init responsibilities. A saved configuration follows Save -> Apply -> Restart -> wait-ready. A disabled service is a no-op and removes its plugin cron entry.
