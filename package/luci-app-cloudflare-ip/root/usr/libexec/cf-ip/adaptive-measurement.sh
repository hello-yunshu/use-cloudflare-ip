...
cfip_adaptive_write_plan() {
    ...
    local tail exploration ranked_fill seed hash rotation run_sequence
    ...
    jq -cn --argjson orders "$orders" --argjson selected "$selected" --argjson remaining "$remaining" --argjson anchors "$anchors" --argjson exploration "$exploration" --argjson ranked "$ranked_fill" --argjson k "$k" --argjson offset "$rotation" --argjson run "$run_sequence" --arg mode "${CFIP_ADAPTIVE_EFFECTIVE_MODE:-shadow}" --argjson ratio "$ratio" --argjson min "$min" --argjson max "$max" \
      '$orders+{selectedK:$k,selectedIps:$selected,remainingCandidates:$remaining,mandatoryAnchorIps:$anchors,explorationIps:$exploration,rankedFillIps:$ranked,explorationOffset:$offset,runSequence:$run,effectiveMode:$mode,targetRatioPercent:$ratio,minProbeCount:$min,maxProbeCount:$max,mandatoryAnchors:$anchors}' | cfip_atomic_write "$output"
}
...
cfip_adaptive_make_audit_record() {
    ...
    local candidate_count actual_count unique_count winner topn planned candidate_ips probed_ips production_probed production_selected k inside top_inside quality severe insuff savings subset_safe_count k25 k40 k60 audit_complete audit_censored censor_reason full_json native_json
    [[ -s "$plan" ]] || return 1
    ...
    subset_safe_count="$(jq -nr --argjson full "$full_json" --argjson planned "$planned" '[ $full[] | select(.ip as $ip | ($planned|index($ip))) | select(.eligible==true and ((.probeSummary.totalMs // 1e18)<=5000) and ((.probeSummary.ttfbMs // 1e18)<=3000) and ((.probeSummary.lossRate // .lossRate // 1)<=0.25)) ] | unique_by(.ip) | length')"
    ...
    jq -cn --arg runId "${CFIP_RUN_ID:-}" --arg fp "${CFIP_ADAPTIVE_CONTEXT_FINGERPRINT:-}" --argjson at "${CFIP_ADAPTIVE_NOW:-0}" --argjson count "$candidate_count" --argjson actual "$actual_count" --argjson baseline "$(jq -c '.baselineOrder' "$plan")" --argjson cfst "$(jq -c '.cfstOrder' "$plan")" --argjson adaptive "$(jq -c '.adaptiveOrder' "$plan")" --argjson planned "$planned" --argjson probed "$production_probed" --argjson production "$production_selected" --arg winner "$winner" --argjson topn "$topn" --argjson k "$k" --argjson inside "$inside" --argjson topInside "$top_inside" --argjson quality "$quality" --argjson severe "$severe" --argjson insuff "$insuff" --argjson subsetSafe "$subset_safe_count" --argjson savings "$savings" --argjson complete "$audit_complete" --argjson censored "$audit_censored" --arg censor "$censor_reason" --argjson scheduler "$CFIP_ADAPTIVE_SCHEDULER_VERSION" --argjson contract "$CFIP_ADAPTIVE_FEATURE_CONTRACT_VERSION" --argjson k25 "$k25" --argjson k40 "$k40" --argjson k60 "$k60" \
      '{schemaVersion:1,fullAudit:true,auditRun:true,auditComplete:$complete,auditCensored:$censored,censorReason:(if $censor=="" then null else $censor end),runId:$runId,at:$at,contextFingerprint:$fp,candidateCount:$count,actualProbeCount:$actual,uniqueProbeCount:($probed|length),baselineOrder:$baseline,cfstOrder:$cfst,adaptiveOrder:$adaptive,adaptivePlannedCandidates:$planned,productionProbedCandidates:$probed,productionSelectedCandidates:$production,appliedCandidates:$production,fullNativeWinner:(if $winner=="" then null else $winner end),fullNativeTopN:$topn,selectedK:$k,adaptivePlannedCandidateRecall:(if $inside then 1 else 0 end),bestCandidateWithinK:$inside,winnerRecall:(if $inside then 1 else 0 end),topNRecall:$topInside,appliedCandidateRecall:(if ($production|index($winner)) then 1 else 0 end),qualityDeltaToFullWinner:$quality,severeMiss:$severe,subsetSafeCount:$subsetSafe,eligibleInsufficiency:($insuff==1),eligibleInsufficiencyRate:$insuff,probeSavings:$savings,K25:$k25,K40:$k40,K60:$k60,schedulerVersion:$scheduler,featureContractVersion:$contract}' | cfip_atomic_write "$output"
}
...
cfip_adaptive_status_json() {
    local state evidence current aggregate last_audit next_due context
    ...
    jq -cn --argjson state "$state" --argjson evidence "$evidence" --argjson aggregate "$aggregate" --argjson last "$last_audit" --argjson next "$next_due" --arg requested "${CFIP_ADAPTIVE_MEASUREMENT_MODE:-off}" --arg effective "$(cfip_adaptive_effective_mode)" \
      '$state+{requestedMode:$requested,effectiveMode:$effective,evidenceWindowSize:($evidence|length),evidenceFreshAt:([$evidence[]|.at]|max // null),available:true,partition:"native-adaptive-measurement",aggregate:$aggregate,lastFullAudit:$last,nextAuditInRuns:$next}'
}
