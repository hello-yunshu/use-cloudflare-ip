...
cfip_rill_state_generation() {
    ...
    jq -r '([.partitions[] | select(.clientIdentityName=="cloudflare-ip" and .partitionKey=="default")][0].handlerSnapshot.stateGeneration // 0)' "$CFIP_RILL_STATE" 2>/dev/null || printf '0'
}
...
cfip_rill_status_json() {
    ...
    request="$(jq -cn --arg id "status-$CFIP_RUN_ID" --arg schema "$schema" '{requestId:$id,apiVersion:3,clientIdentity:{name:"cloudflare-ip",version:"2.0.0"},partitionKey:"default",featureSchemaHash:$schema,modelGeneration:1,stateGeneration:0,payloadLimit:1048576,request:{method:"handshake"}}')"
...
cfip_rill_rank_shadow() {
    ...
    jq -cn --arg id "decision-$CFIP_RUN_ID" --arg schema "$schema" --argjson generation "$(cfip_rill_state_generation)" --argjson actions "$actions" \
      '{requestId:$id,apiVersion:3,clientIdentity:{name:"cloudflare-ip",version:"2.0.0"},partitionKey:"default",capability:"org.rill.preview.decide",featureSchemaHash:$schema,modelGeneration:1,stateGeneration:$generation,payloadLimit:1048576,request:{method:"decide",context:{actions:$actions}}}' >"$request"
